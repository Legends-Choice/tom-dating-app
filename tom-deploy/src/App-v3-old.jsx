...
const api = {
  ...
  async login({ email, password }) {
    ...
    const { data: row, error: profErr } = await supabase.from("profiles").select("*").eq("id", data.user.id).single();
    if (profErr || !row) return { error: "We couldn't find your profile. Try signing up again." };
      ...
      <div style={{ display: "flex", gap: 10, padding: "14px 22px 20px" }}>
        ...
        <div style={{ flex: 1 }}>
          <PrimaryBtn disabled={!canNext || saving} onClick={async () => {
            ...
            {saving ? "Saving..." : (step < 3 ? "Continue" : "Start spending time")}
          </PrimaryBtn>
        </div>
      </div>
    </div>
...
export default function TomApp() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      const [d, m] = await Promise.all([api.loadDeck(), api.loadMatches()]);
      setDeck(d);
      setMatches(m);
    })();
  }, [screen]);
